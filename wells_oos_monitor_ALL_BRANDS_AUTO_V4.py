...
def extract_discount(page) -> str:
    ...
    try:
        ...
        () => {
            ...
            // Fallback: calcula % a partir de preco vs PVPR
    ...
    if len(variants) <= 1:
        ...
        for tile in variants:
            ...
            try:
                ...
                _GET_BADGE_JS = """() => {
                    var sels = ['.js-w-circle-badge .w-badge-price','.w-circle-product-badge .w-badge-price','.w-circle-badge .w-badge-price','.js-w-circle-badge','.w-circle-product-badge','.w-circle-badge'];
                    for (var s of sels) {
                        var b = document.querySelector(s);
                        if (b) { var t=(b.innerText||b.textContent||'').replace(/\\s+/g,' ').trim(); var m=t.match(/(\\d+)\\s*%/); if(m) return m[1]+'%'; }
                    }
                    var txt=document.body.innerText||'';
                    var mp=txt.match(/(\\d+[,.]\\d+)\\s*€?\\s*(\\d+[,.]\\d+)\\s*PVPR/i);
                    if(mp){var p=parseFloat(mp[1].replace(',','.'));var v=parseFloat(mp[2].replace(',','.'));if(v>p&&v>0)return Math.round((v-p)/v*100)+'%';}
                    return '';
                }"""
                try:
                ...
                # Aguarda badge actualizar para valor DIFERENTE do anterior (ou ate 3s)
                _badge = _badge_before
                for _d in range(15):
                    page.wait_for_timeout(200)
                    try:
                        ...
                        _badge_now = ''
                    if _badge_now and _badge_now != _badge_before:
                        _badge = _badge_now
                        break
                    elif _badge_now:
                        _badge = _badge_now  # actualiza mas continua a esperar mudança
