    ...
    () => {
      ...
      return tiles.map(a => {
        ...
        return {
          label:    (a.getAttribute('data-attr-value') || a.innerText || '').trim(),
          href:     a.href || '',
          selected: a.classList.contains('select-capacity') ||
                    a.classList.contains('selected') ||
                    a.classList.contains('active') ||
                    a.classList.contains('js-selected') ||
                    a.classList.contains('w-selected') ||
                    a.getAttribute('aria-selected') === 'true' ||
                    a.getAttribute('aria-checked') === 'true' ||
    ...
    if len(variants) <= 1:
        ...
        for tile in variants:
            ...
            try:
                ...
                # Se já estava seleccionada (variante por defeito), o click não dispara AJAX
                # — detectamos isso verificando se a REF mudou
                _already_selected = tile.get("selected", False)
                if _already_selected:
                    log(f"      [VARIANTE] '{tile_label}' já seleccionada (DOM), lê desconto directo")
                    ...
                    for _w in range(6):
                        ...
                        if candidate and candidate != last_ref:
                            ...
                            break
                    if not _ref_changed:
                        # REF não mudou — provavelmente já estava seleccionada
                        ref_v = extract_ref(page) or ref_base
                        log(f"      [VARIANTE] '{tile_label}' REF não mudou após click (já seleccionada?)")
                    last_ref = ref_v
