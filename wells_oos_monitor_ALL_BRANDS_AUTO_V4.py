...
def extract_discount(page) -> str:
    ...
    try:
        ...
        () => {
            ...
            if (badge) {
                const num  = badge.querySelector('.w-discount');
                const unit = badge.querySelector('.w-unit');
                if (num) {
                    const n = (num.innerText || num.textContent || '').trim();
                    const u = unit ? (unit.innerText || unit.textContent || '').trim() : '%';
                    if (n && /^\\d+$/.test(n)) return n + (u || '%');
                }
    ...
    if len(variants) <= 1:
        ...
        for tile in variants:
            ...
            try:
                ...
                # Aguarda badge de desconto estabilizar apos AJAX
                # Timeout aumentado para GitHub Actions (maquina mais lenta)
                for _d in range(12):
                    page.wait_for_timeout(250)
                    _badge = page.evaluate("""() => {
                        var b = document.querySelector('.js-w-circle-badge .w-badge-price, .w-circle-product-badge .w-badge-price, .w-circle-badge .w-badge-price');
                        return b ? (b.innerText || b.textContent || '').trim() : '';
                    }""")
                    if _badge:
...
function renderKPIs(rows) {
  ...
  var ok    = total - oos;
  // Sem desconto: conta por URL (produto), não por variante
  // Só conta se NENHUMA variante do produto tem desconto
  var urlDescs = {};
  DATA.forEach(function(r) {
    var url = r.url || "";
    if (!urlDescs[url]) urlDescs[url] = [];
    urlDescs[url].push(r.desconto || "");
  });
  var noDesc = Object.keys(urlDescs).filter(function(url) {
    return urlDescs[url].every(function(d) { return !d || d === "Sem desconto"; });
  }).length;
