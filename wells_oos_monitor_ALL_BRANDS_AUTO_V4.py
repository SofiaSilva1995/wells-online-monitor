...
def extract_discount(page) -> str:
    ...
    Selector confirmado no DOM do Wells:
      .js-w-circle-badge .w-badge-price
        span.w-discount  -> numero (ex: "25")
        span.w-unit      -> "%"
    ...
    try:
        ...
        () => {
            // Tenta todos os selectores possiveis de badge de desconto
            const selectors = [
                '.js-w-circle-badge .w-badge-price',
                '.w-circle-product-badge .w-badge-price',
                '.w-circle-badge .w-badge-price',
                '.js-w-circle-badge',
                '.w-circle-product-badge',
                '.w-circle-badge',
                '[class*="circle-badge"]',
                '[class*="badge-price"]',
                '[class*="badge-promo"]',
            ...
            for (const sel of selectors) {
                ...
                if (!badge) continue;
                // Tenta extrair numero% do texto completo do badge
        ...
        # Fallback: calcula % desconto a partir do preco actual vs PVPR
        try:
            pct = page.evaluate("""
            () => {
                const txt = document.body.innerText || '';
                // Procura padrao "€X,XX €Y,YY PVPR" ou "€X,XX PVPR"
                const m = txt.match(/[€]?\\s*(\\d+[.,]\\d+)\\s*[€]?\\s*(\\d+[.,]\\d+)\\s*PVPR/i);
                if (m) {
                    const preco = parseFloat(m[1].replace(',','.'));
                    const pvpr  = parseFloat(m[2].replace(',','.'));
                    if (pvpr > preco && pvpr > 0) {
                        return Math.round((pvpr - preco) / pvpr * 100) + '%';
                    }
                }
                // Fallback: se tem PVPR mas nao conseguiu calcular
                if (txt.toLowerCase().includes('pvpr')) return 'desc';
                return '';
            }
            """)
            if pct:
                log(f"      [DESCONTO] calculado via PVPR: {pct}")
                return pct
        except Exception:
            pass
...
def check_product(page, url: str, brand_label: str) -> List[Dict]:
    ...
    if len(variants) <= 1:
        ...
        try:
            html = page.evaluate("() => document.body.innerText.toLowerCase()")
        except Exception:
            html = ""
        is_oos = 1 if any(k in html for k in OOS_KEYWORDS) else 0
        ...
        # Fallback para badge SOBRE PVPR e similares
        if desconto == "Sem desconto":
            try:
                txt = page.evaluate("() => document.body.innerText")
                import re as _re
                m = _re.search(r'(\d+)%\s*(?:SOBRE|sobre|desconto|DESCONTO)', txt)
                if m:
                    desconto = m.group(1) + "%"
            except Exception:
                pass
        ...
        for tile in variants:
            ...
            try:
                ...
                # Aguarda AJAX - tenta ate 10x com 200ms ate REF mudar
                ref_v = ref_base
                for _w in range(6):
                    page.wait_for_timeout(150)
                ...
                last_ref = ref_v  # actualiza para o proximo click
                # Aguarda badge de desconto actualizar apos clique na variante
                # Espera que o valor do badge MUDE face ao anterior OU estabilize
                _get_badge = """() => {
                    var selectors = [
                        '.js-w-circle-badge .w-badge-price',
                        '.w-circle-product-badge .w-badge-price',
                    ...
                    // Fallback PVPR: calcula % a partir dos precos visiveis
                    var txt = document.body.innerText || '';
                    var mp = txt.match(/[€]?\\s*(\\d+[,.]\\d+)\\s*€?\\s*(\\d+[,.]\\d+)\\s*PVPR/i);
                    if (mp) {
                        var preco = parseFloat(mp[1].replace(',','.'));
                        var pvpr  = parseFloat(mp[2].replace(',','.'));
                        if (pvpr > preco && pvpr > 0) return Math.round((pvpr-preco)/pvpr*100) + '%';
                    }
                    return '';
                }"""
                _prev_badge = _badge if '_badge' in dir() else ''
                for _d in range(15):
                    page.wait_for_timeout(300)
                    _badge = page.evaluate(_get_badge)
                    if _badge and _badge != _prev_badge:
                        break
                # Extrai desconto apos AJAX (pode variar por variante)
                desconto_v = extract_discount(page)
                # Fallback: se nao encontrou desconto, tenta via texto da pagina
                if desconto_v == "Sem desconto":
                    try:
                        txt = page.evaluate("() => document.body.innerText")
                        import re as _re
                        m = _re.search(r'(\d+)%\s*(?:SOBRE|sobre|desconto|DESCONTO)', txt)
                        if m:
                            desconto_v = m.group(1) + "%"
                    except Exception:
                        pass
                # Se DOM nao foi conclusivo, usa texto visivel (mais rapido que page.content())
                if dom_oos == -1:
                    try:
                        txt_v = page.evaluate("() => document.body.innerText.toLowerCase()")
                        dom_oos = 1 if any(k in txt_v for k in OOS_KEYWORDS) else 0
                    except Exception:
                        dom_oos = 0
            except Exception:
                if dom_oos == -1:
...
def run_brand(cfg: Dict, page) -> Dict:
    ...
    run_xlsx = os.path.join(logs_dir, f"verificacao_{run_id}.xlsx")

    log(f"--- {label} | Start ---")
    known_oos = load_known(known_json)
    first_run = (len(known_oos) == 0)

    # ---- Paginar listagem e recolher URLs base ----
    ...
    # all_urls: URLs base da listagem
    # variant_candidates: URLs que precisam de expansao (tem "+X Tamanho(s)")
    ...
    log(f"{label}: OOS atuais: {len(current_oos)} | NOVOS OOS: {len(new_oos)}")
    ...
    return {
        ...
        "all_rows":        all_rows,   # lista de dicts para Excel consolidado
        "headers":         HEADERS,
    }
...
def write_consolidated_xlsx(xlsx_path: str, results: List[Dict]) -> None:
    ...
    for r in results:
        # Sem desconto: produtos (por URL) onde NENHUMA variante tem desconto
        url_descontos = {}
        for row in r.get("all_rows", []):
            url = row.get("url", "")
            desc = row.get("desconto", "") or ""
            if url not in url_descontos:
                url_descontos[url] = set()
            url_descontos[url].add(desc.strip())
        sem_desc = sum(
            1 for url, descs in url_descontos.items()
            if all(d == "" or d == "Sem desconto" for d in descs)
        )
        ws_resumo.append([
            r["label"],
            r["products_found"],
    ...
    # ---- Sheet de Variantes ----
    # Todos os produtos que tem mais de 1 linha com o mesmo URL (ou seja, tem variantes)
    from collections import defaultdict
    url_rows = defaultdict(list)
    for r in results:
        for row in r.get("all_rows", []):
            url_rows[row.get("url","")].append(row)

    variant_rows = []
    for url, rows_for_url in url_rows.items():
        if len(rows_for_url) > 1:
            variant_rows.extend(rows_for_url)

    if variant_rows:
        ws_var = wb.create_sheet(title="Variantes")
        var_headers = ["marca", "titulo", "url", "nome_variante", "is_oos", "ref_produto", "desconto"]
        var_display = ["Marca", "Produto", "URL", "Variante", "Estado", "REF", "Desconto"]
        ws_var.append(var_display)
        for c in range(1, len(var_display)+1):
        ...
        prev_url_v = None
        use_grey_v = False
        for row_dict in variant_rows:
            cur_url_v = row_dict.get("url","")
            if cur_url_v != prev_url_v:
                use_grey_v = not use_grey_v
                prev_url_v = cur_url_v
            is_oos = row_dict.get("is_oos", 0)
            row_vals = [row_dict.get(h,"") for h in var_headers]
            ws_var.append(row_vals)
            row_idx = ws_var.max_row
            for c in range(1, len(var_headers)+1):
                ...
                if is_oos:
                    ...
                    cell.fill = GREY_FILL
            # Estado cell
            estado_col = var_headers.index("is_oos") + 1
            cell_e = ws_var.cell(row=row_idx, column=estado_col)
            cell_e.value = "Out of Stock" if is_oos else "In Stock"
            cell_e.font = OOS_FONT_RED if is_oos else OOS_FONT_GREEN
            cell_e.alignment = Alignment(horizontal="center", vertical="center")

        var_widths = {"marca":14,"titulo":40,"url":55,"nome_variante":18,"is_oos":16,"ref_produto":14,"desconto":12}
        for c, h in enumerate(var_headers, 1):
            ws_var.column_dimensions[get_column_letter(c)].width = var_widths.get(h, 15)

        last_row_v = len(variant_rows) + 1
        last_col_v = get_column_letter(len(var_headers))
        if last_row_v > 1:
            tbl_v = Table(displayName="Variantes", ref=f"A1:{last_col_v}{last_row_v}")
            style_v = TableStyleInfo(name="TableStyleMedium9", showFirstColumn=False,
                showLastColumn=False, showRowStripes=False, showColumnStripes=False)
            tbl_v.tableStyleInfo = style_v
            ws_var.add_table(tbl_v)
...
<div class="header">
  ...
  <div class="header-text">
    ...
    <div class="sub">Avène &nbsp;·&nbsp; Ducray &nbsp;·&nbsp; Klorane &nbsp;·&nbsp; René Furterer &nbsp;·&nbsp; A-Derma &nbsp;·&nbsp; Wells.pt</div>
  </div>
...
<div class="wrap">
  ...
  <div class="sec">Evolução Out of Stock</div>
        ...
        <thead id="tHead">
          <tr>
            <th><div class="th-inner">
              ...
              <select class="col-filter" id="cf-data"><option value="">Todas</option></select>
            </div></th>
            <th><div class="th-inner">
...
function tableFiltered(base) {
  ...
  var cfD = $("cf-data").value.toLowerCase();
  ...
  return base.filter(function(r) {
    ...
    if (cfD && r.data !== cfD) return false;
...
function renderKPIs(rows) {
  ...
  var ok    = total - oos;
  // Sem desconto: conta por linha (consistente com kTotal que também conta por linha)
  var noDesc = DATA.filter(function(r) {
...
$("kpi-nodesc").addEventListener("click", function() {
  ...
  $("cf-estado").value=""; $("cf-ref").value=""; $("cf-data").value=""; $("cf-desconto").value=""; $("cf-oos-dias").value="";
...
$("btnResetTable").addEventListener("click", function() {
  _newOosFilter = false; _semDescontoFilter = false;
  $("cf-marca").value=""; $("cf-titulo").value=""; $("cf-variante").value="";
  $("cf-estado").value=""; $("cf-ref").value=""; $("cf-data").value="";
...
function populateDateFilter(base) {
  var dates = [...new Set(base.map(function(r){ return r.data; }))].filter(Boolean).sort().reverse();
  var cf = $("cf-data"), cur = cf.value;
  cf.innerHTML = "<option value=''>Todas</option>";
...
tableFiltered = function(base) {
  var rows;
  if (_newOosFilter) {
    // Ignora filtros globais — mostra exactamente os novos OOS
...
def build_oos_desde_map(historico: list, current_rows: list) -> dict:
    """
    Calcula para cada REF a data em que ficou OOS pela primeira vez.
    Retorna dict {ref_produto -> data_oos_desde}.
    """
    oos_desde = {}
    # Do historico anterior
    for row in historico:
        if int(row.get("is_oos", 0)):
            ref = row.get("ref_produto", "")
            if ref and ref not in oos_desde:
                oos_desde[ref] = row.get("oos_desde") or row.get("data", "")
    # Da run actual (novos OOS que nao estavam no historico)
...
def generate_dashboard(run_id: str, all_results: list, output_path: str,
    ...
    # Refs dos novos OOS — usa exactamente os mesmos URLs que new_count
    # para garantir que NEW_OOS_COUNT == len(NEW_OOS_REFS)
    new_oos_urls = set()
    for r in all_results:
        for url in r.get("new_oos", []):
            new_oos_urls.add(url)

    new_oos_refs = []
    for r in all_results:
        for row in r.get("all_rows", []):
            if row.get("is_oos") and row.get("url") in new_oos_urls:
                ref = row.get("ref_produto", "")
...
def main() -> None:
    ...
    for r in results:
        ...
        body_lines.append(f"  OOS atuais: {r['current_count']} | NOVOS OOS: {r['new_count']}\n")

        if r["first_run"] and r["current_count"] > 0:
            send = True
    ...
    consolidated_xlsx = os.path.join("logs_consolidado", f"todas_marcas_{master_run_id}.xlsx")
    ...
    os.makedirs("logs", exist_ok=True)
    dashboard_html       = os.path.join("logs_consolidado", f"dashboard_{master_run_id}.html")
    ...
    try:
        ...
        # Copia para logs/ para ficar no repositorio GitHub
        import shutil as _shutil
        _shutil.copy2(dashboard_html, dashboard_html_repo)
        log(f"Dashboard HTML: {dashboard_html}")
        log(f"Dashboard copiado para repo: {dashboard_html_repo}")
    except Exception as e:
    ...
    if send:
        ...
        subject = "Stock Wells Online - NOVOS OOS (Avène / Ducray / Klorane / René Furterer / A-Derma)"
        ...
        if consolidated_xlsx and os.path.exists(consolidated_xlsx):
            try:
                tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx", prefix="todas_marcas_")
                tmp.close(); shutil.copy2(consolidated_xlsx, tmp.name)
                uniq.append(tmp.name); tmp_copies.append(tmp.name)
            except Exception:
                uniq.append(consolidated_xlsx)
        # 2. Dashboard HTML
        if dashboard_html and os.path.exists(dashboard_html):
            try:
                tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".html", prefix="dashboard_OOS_")
                tmp.close(); shutil.copy2(dashboard_html, tmp.name)
                uniq.append(tmp.name); tmp_copies.append(tmp.name)
            except Exception:
            ...
            if a and a not in seen and os.path.exists(a):
                ...
                try:
                    tmp = tempfile.NamedTemporaryFile(
                        ...
                        prefix=os.path.basename(a).replace(".xlsx", "_")
                    )
                    tmp.close(); shutil.copy2(a, tmp.name)
                    uniq.append(tmp.name); tmp_copies.append(tmp.name)
                except Exception:
                    uniq.append(a)
