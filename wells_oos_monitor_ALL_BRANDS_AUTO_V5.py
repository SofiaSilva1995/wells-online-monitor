...
def generate_dashboard(run_id: str, all_results: list, output_path: str,
    ...
    # "Novo OOS real" = oos_desde == hoje
    # Regra: se um produto volta ao stock e fica OOS novamente, o oos_desde reseta para hoje
    # Isso já é tratado pelo build_oos_desde_map — basta usar oos_desde como fonte de verdade
    ...
    _today_dt = _pd(_today_str)
    new_oos_refs_set = set(
        r.get("ref_produto", "")
        for r in current_rows
        if r.get("is_oos") and r.get("oos_desde") == _today_str and r.get("ref_produto")
    )
