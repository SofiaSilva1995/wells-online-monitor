...
function renderKPIs(rows) {
  ...
  if (kRemovedEl) {
    ...
    if (kRemovedPct) kRemovedPct.innerHTML = REMOVED > 0
      ? "<span style='color:#b71c1c'>🗑 " + REMOVED + " removidos hoje</span>"
...
function resetFilters() {
  _newOosFilter = false; _goneOosFilter = false; _removedFilter = false; _semDescontoFilter = false;
...
tableFiltered = function(base) {
  var rows;
  if (_newOosFilter) {
    // Pré-filtrar por Novos OOS, depois aplicar filtros de coluna normalmente
    ...
    rows = rows.map(function(r) { return Object.assign({}, r, {_recovered: true}); });
  ...
  // Aplicar filtros de coluna sobre o resultado do card
  return _origTableFiltered(rows);
};
