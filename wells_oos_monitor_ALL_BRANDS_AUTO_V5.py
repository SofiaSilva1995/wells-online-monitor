...
<div class="wrap">
  ...
  <div class="sec-body open" id="bodyResumo"><div class="kpi-grid">
    ...
    <div class="kpi k-new kpi-click" id="kpi-new" title="Ver novos Out of Stock">
      <div class="kpi-lbl" id="kNewLbl">Variação Out of Stock</div>
      <div class="kpi-val" id="kNew">—</div>
      <div class="kpi-sub" id="kNewPct" style="display:flex;align-items:center;gap:6px">vs dia anterior</div>
...
function renderKPIs(rows) {
  ...
  // Card "Variação OOS" — cor e sinal dinâmicos
  var kNewCard = $("kpi-new");
  var kNewLbl  = $("kNewLbl");
  var kNewEl   = $("kNew");
  var kNewPct  = $("kNewPct");
  if (kNewCard && kNewEl) {
    kNewCard.classList.remove("k-new-up","k-new-down","k-new-neutral");
    if (NEW_OOS > 0) {
      kNewCard.classList.add("k-new-up");
      kNewEl.textContent = "+" + NEW_OOS;
      if (kNewLbl) kNewLbl.textContent = "Novos Out of Stock";
      if (kNewPct) kNewPct.innerHTML = "<span style='color:var(--red)'>▲ +" + NEW_OOS + " vs dia anterior</span>";
    } else if (NEW_OOS < 0) {
      kNewCard.classList.add("k-new-down");
      kNewEl.textContent = NEW_OOS;
      if (kNewLbl) kNewLbl.textContent = "Recuperados";
      if (kNewPct) kNewPct.innerHTML = "<span style='color:var(--green)'>▼ " + NEW_OOS + " vs dia anterior</span>";
    } else {
      kNewCard.classList.add("k-new-neutral");
      kNewEl.textContent = "=";
      if (kNewLbl) kNewLbl.textContent = "Variação Out of Stock";
      if (kNewPct) kNewPct.innerHTML = "<span style='color:#6b7a8d'>→ igual ao dia anterior</span>";
    }
  }
...
def generate_dashboard(run_id: str, all_results: list, output_path: str,
    ...
    new_oos_count = today_oos_count - prev_oos_count  # pode ser negativo (melhoria)

    # new_oos_refs: mantém compatibilidade para filtro da tabela (usa oos_desde)
    new_oos_refs = []
    for row in current_rows:
