...
BRANDS: List[Dict] = [
    ...
    {"key": "ducray",       "label": "Ducray",       "base_url": "https://wells.pt/marcas/ducray",       "max_pages_fallback": 10, "exclude_urls": set()},
    {"key": "klorane",      "label": "Klorane",      "base_url": "https://wells.pt/marcas/klorane",      "max_pages_fallback": 12, "exclude_urls": set()},
    {"key": "rene_furterer","label": "René Furterer","base_url": "https://wells.pt/marcas/rene-furterer","max_pages_fallback": 12, "exclude_urls": set()},
    {"key": "a_derma",      "label": "A-Derma",      "base_url": "https://wells.pt/marcas/a-derma",      "max_pages_fallback": 10, "exclude_urls": set()},
    {
        "key": "oral_care",
        "label": "Oral Care",
        "multi_brand": True,
...
def extract_urls_from_dom(page, brand_label: str, exclude_urls: Set[str], expected_min: int = 0) -> Set[str]:
    """
    Recolhe URLs de produto da pagina de listagem atual.
    Nao filtra por texto de marca no card - ja estamos numa pagina
    especifica da marca (ex: wells.pt/avene.html) por isso todos os
    produtos listados sao da marca correta.
    """
...
def check_product(page, url: str, brand_label: str) -> List[Dict]:
    ...
    if len(variants) <= 1:
        ...
        for tile in variants:
            ...
            # Garante que dom_oos nunca é -1 (desconhecido) no CSV/dashboard
            if dom_oos == -1:
                dom_oos = 0
            rows.append({
                "data":          data_hoje,
...
def run_brand(cfg: Dict, page) -> Dict:
    key = cfg["key"]
    label = cfg["label"]
    ...
    if multi_brand:
        ...
        for pnum in page_range:
            ...
            if pnum == 1:
                ...
                list_url = base_url + ("&" if "?" in base_url else "?") + f"srule=product-name-ascending&page={pnum}"
            ...
            if pnum == 1:
                ...
                # Agora adiciona pagina 1 limpa + pagina 2 (sem intrusos)
            ...
            for card in cards:
                ...
                if u and card.get("has_variants") and u in all_urls:
    ...
    # Nota: new_oos compara URLs entre runs (para email/known_oos).
    # O dashboard usa comparação por ref_produto via historico.csv — é a fonte de verdade.
    log(f"{label}: OOS atuais: {len(current_oos)} produtos / {current_count_variants} variantes | NOVOS OOS: {len(new_oos)} produtos / {new_oos_count_variants} variantes")
...
<title>Pierre Fabre · Wells Online Monitor</title>
...
.kpi.k-new.k-new-up::before{background:var(--red)}
.kpi.k-new.k-new-down::before{background:var(--green)}
.kpi.k-new.k-new-neutral::before{background:#6b7a8d}
.kpi.k-nodesc::before{background:#6a1b9a}
.kpi.k-nodesc .kpi-val{color:#6a1b9a}
.kpi.k-removed::before{background:#7b1fa2}
.kpi.k-removed .kpi-val{color:#7b1fa2}
...
.kpi.k-new.k-new-up   .kpi-val{color:var(--red)}
.kpi.k-new.k-new-down .kpi-val{color:var(--green)}
.kpi.k-new.k-new-neutral .kpi-val{color:#6b7a8d}
...
.badge-recovered{display:inline-flex;align-items:center;gap:4px;background:#e3f2fd;color:#1565c0;
  padding:2px 10px;border-radius:20px;font-size:11px;font-weight:700;border:1px solid #90caf9}
.recovered-row{background:#f0f7ff !important}
.recovered-row:hover{background:#dbeeff !important}
.badge-removed{display:inline-flex;align-items:center;gap:4px;background:#fce4ec;color:#b71c1c;
  padding:2px 10px;border-radius:20px;font-size:11px;font-weight:700;border:1px solid #ef9a9a}
.removed-row{background:#fff5f5 !important}
.removed-row:hover{background:#ffe8e8 !important}
...
@media (max-width: 768px) {
  ...
  .pf-logo-wrap{width:38px;height:38px}
  .pf-logo-wrap img{width:38px;height:38px}
  .header-text h1{font-size:14px;letter-spacing:-.3px}
  .header-text .sub{font-size:10px;display:none}
  .header-right{display:none}
  .wrap{padding:12px 12px 56px}
  .sec{font-size:10px;margin:18px 0 10px}
  ...
  .bc-stats{font-size:10px}
  .filters{flex-direction:column;gap:10px;padding:14px}
  .fg{min-width:unset;width:100%}
  .fg select,.fg input[type=text]{width:100%;font-size:12px}
  .fg-date-pair{width:100%}
  .date-row{flex-direction:column;align-items:flex-start;gap:6px}
  .date-row input{width:100%}
  .chk{width:100%}
  .btn-reset,.btn-export{width:100%}
  .chart-card{padding:14px}
  .chart-wrap{height:200px}
  .radar-inner{grid-template-columns:1fr;gap:16px;padding-top:14px}
  .radar-donut-wrap{width:160px;height:160px;margin:0 auto}
  .radar-band-row{grid-template-columns:80px 1fr 40px 36px;gap:6px;padding:6px}
  .tbl-head{flex-direction:column;align-items:flex-start;gap:8px;padding:10px 14px}
  .tbl-wrap{-webkit-overflow-scrolling:touch}
  table{font-size:11px}
  .th-inner{padding:7px 8px 6px;gap:4px}
  .th-lbl{font-size:9px}
  .col-filter,.col-filter-txt{height:22px;font-size:10px;padding:2px 5px;min-width:50px}
  tbody td{padding:7px 8px;font-size:11px}
  td.ttl-col{max-width:160px}
  .badge-oos,.badge-ok,.badge-recovered,.badge-removed{font-size:10px;padding:2px 7px}
  .pagination{gap:6px;padding:10px}
  .btn-pg{padding:4px 10px;font-size:11px}
  .pg-info{font-size:11px}
}
@media (max-width: 480px) {
  .header{padding:9px 12px}
  .header-text h1{font-size:12px}
  .wrap{padding:10px 10px 52px}
  .kpi-grid{grid-template-columns:1fr 1fr;gap:8px}
  .kpi{padding:10px 12px 8px}
  .kpi-val{font-size:22px}
  .brand-grid{grid-template-columns:1fr 1fr;gap:6px}
  ...
  .radar-donut-wrap{width:140px;height:140px}
}
...
<div class="header">
  ...
  <div class="header-text">
    <h1>Pierre Fabre · Wells Online Monitor</h1>
    <div class="sub">Avène &nbsp;·&nbsp; Ducray &nbsp;·&nbsp; Klorane &nbsp;·&nbsp; René Furterer &nbsp;·&nbsp; A-Derma &nbsp;·&nbsp; Dexeryl &nbsp;·&nbsp; Oral Care &nbsp;·&nbsp; Wells.pt</div></div>
  </div>
...
<div class="wrap">
  ...
  <div class="sec-body open" id="bodyResumo"><div class="kpi-grid">
    ...
    <div class="kpi k-new k-new-up kpi-click" id="kpi-new" title="Ver novos Out of Stock">
      <div class="kpi-lbl">Novos Out of Stock</div>
      <div class="kpi-val" id="kNew">—</div>
      <div class="kpi-sub" id="kNewPct">vs dia anterior</div>
    </div>
  ...
  <div class="sec-body open" id="bodyFiltros"><div class="filters">
    ...
    <div class="fg">
      ...
      <select id="fEstado">
        ...
        <option value="d">Removido</option>
      </select>
    </div>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:16px">
      ...
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        <select id="chartMonthFilter"
          style="padding:4px 10px;height:28px;border-radius:8px;border:1.5px solid var(--blue-mid);
                 background:var(--blue-pale);color:var(--blue);font-size:12px;font-weight:700;
                 font-family:inherit;cursor:pointer;outline:none">
          <option value="all">Todos os meses</option>
        </select>
        <button id="btnChartMTD"
          style="padding:4px 12px;height:28px;border-radius:8px;background:var(--card);color:var(--muted);
                 border:1.5px solid var(--line);font-size:12px;font-weight:700;font-family:inherit;cursor:pointer;transition:all .15s">
          MTD
        </button>
        <button id="btnChartReset"
          style="padding:4px 14px;height:28px;border-radius:8px;background:var(--blue-pale);color:var(--blue);
                 border:1.5px solid var(--blue-mid);font-size:12px;font-weight:700;font-family:inherit;cursor:pointer;transition:background .15s">
          ↺ Mostrar todas
        </button>
      </div>
    </div>
  ...
  <div class="sec-body open" id="bodyProdutos"><div class="table-card">
    <div class="tbl-head">
      ...
      <div style="display:flex;align-items:center;gap:10px">
        <span class="tbl-count" id="rowCount">— resultados</span>
        <button class="btn-reset" id="btnResetTable" style="height:30px;padding:4px 14px;font-size:12px">↺ Limpar filtros</button>
...
var GONE_OOS   = {{GONE_OOS_COUNT}};
var REMOVED    = {{REMOVED_COUNT}};
var REMOVED_REFS = {{REMOVED_REFS_JSON}};
var NEW_OOS_REFS_SET  = null;  // preenchido abaixo
var GONE_OOS_REFS_SET = null;  // preenchido abaixo
var DATA = {{DATA_JSON}};
DATA = DATA.map(function(r){ if(r.is_removed) return Object.assign({},r,{_removed:true}); return r; });
...
function globalFiltered() {
  ...
  return DATA.filter(function(r) {
    ...
    if (fE === "r" && !r._recovered) return false;
    if (fE === "d" && !r._removed) return false;
    if (fE !== "" && fE !== "r" && fE !== "d" && String(r.is_oos) !== fE) return false;
...
function tableFiltered(base) {
  ...
  return base.filter(function(r) {
    ...
    if (cfE === "r" && !r._recovered) return false;
    if (cfE === "d" && !r._removed) return false;
    if (cfE !== "" && cfE !== "r" && cfE !== "d" && String(r.is_oos) !== cfE) return false;
...
function renderKPIs(rows) {
  ...
  var kNewPct = $("kNewPct");
  if (kNewEl) {
    kNewEl.textContent  = NEW_OOS > 0 ? "+" + NEW_OOS : (NEW_OOS === 0 ? "=" : NEW_OOS);
    if (kNewPct) kNewPct.innerHTML = NEW_OOS > 0
      ? "<span style='color:var(--red)'>▲ +" + NEW_OOS + " vs dia anterior</span>"
      : "<span style='color:#6b7a8d'>→ igual ao dia anterior</span>";
  }
  // Card "Recuperados"
  var kGoneEl  = $("kGone");
  var kGonePct = $("kGonePct");
  if (kGoneEl) {
    kGoneEl.textContent  = GONE_OOS > 0 ? "-" + GONE_OOS : (GONE_OOS === 0 ? "=" : GONE_OOS);
    if (kGonePct) kGonePct.innerHTML = GONE_OOS > 0
      ? "<span style='color:var(--green)'>▼ -" + GONE_OOS + " vs dia anterior</span>"
  ...
  if (kRemovedEl) {
    kRemovedEl.textContent = REMOVED > 0 ? "🗑 " + REMOVED : "=";
    if (kRemovedPct) kRemovedPct.innerHTML = REMOVED > 0
      ? "<span style='color:#7b1fa2'>🗑 " + REMOVED + " removidos hoje</span>"
  ...
  // (kNewPct já tratado no bloco de renderização do card acima)
...
function renderBrands() {
  ...
  var marcas = unique(DATA.map(function(r) { return r.marca; })).sort(function(a,b){
  ...
  marcas.forEach(function(m) {
    var rows = DATA.filter(function(r) { return r.marca === m; });
    var oos  = rows.filter(function(r) { return r.is_oos; }).length;
...
function renderTable() {
  ...
  if (slice.length === 0) {
    ...
    tbody.innerHTML = slice.map(function(r) {
      ...
      var rowCls = diasOos(r.oos_desde) > 30 ? "oos-critical" : r.is_oos ? "oos-row" : "";
...
$("kpi-total").addEventListener("click", function() {
  $("fEstado").value = ""; $("fSoOos").checked = false;
  page = 1; render();
  setTimeout(scrollToTable, 100);
});
...
$("kpi-nodesc").addEventListener("click", function() {
  _newOosFilter = false; _goneOosFilter = false; _semDescontoFilter = false;
...
var GONE_OOS_REFS  = {{GONE_OOS_REFS_JSON}};
var REMOVED_REFS   = {{REMOVED_REFS_JSON}};
...
tableFiltered = function(base) {
  var rows;
  if (_newOosFilter) {
    // Mostra exactamente os novos OOS (entrou em OOS desde ontem)
    rows = DATA.filter(function(r) {
      return NEW_OOS_REFS.indexOf(r.ref_produto) !== -1;
    });
  } else if (_goneOosFilter) {
    // Mostra exactamente os recuperados (saiu de OOS desde ontem)
    // Estes produtos estão no histórico do dia anterior mas JÁ NÃO estão OOS hoje
    // Mostramos como linhas in-stock (is_oos=0) mas com destaque
    rows = DATA.filter(function(r) {
      return GONE_OOS_REFS.indexOf(r.ref_produto) !== -1;
    });
    // Se o produto recuperado já não aparece no DATA de hoje, não temos linha para mostrar
    // Nesse caso mostramos uma mensagem — tratado no renderTable via rows vazio
  } else if (_removedFilter) {
    rows = DATA.filter(function(r) {
      return r._removed === true;
    });
...
/* ── CHART ── */
var _chart = null;
function renderChart(filteredDates) {
  ...
  if (filteredDates && filteredDates.length) {
    var filteredSet = new Set(filteredDates);
    allDates = allDates.filter(function(d){ return filteredSet.has(d); });
    allRows  = allRows.filter(function(r){ return filteredSet.has(r.data); });
  }
...
document.getElementById("btnChartReset").addEventListener("click", function() {
  // Reset filtro de mês
  var sel = document.getElementById("chartMonthFilter");
  if (sel) sel.value = "all";
  var btnMTD = document.getElementById("btnChartMTD");
  if (btnMTD) { btnMTD.style.background="var(--card)"; btnMTD.style.color="var(--muted)"; btnMTD.style.borderColor="var(--line)"; }
  renderChart();
  if (_chart) { var n=_chart.data.datasets.length; for(var i=0;i<n;i++) _chart.show(i); _chart.update(); }
});
document.getElementById("btnChartReset").addEventListener("mouseover", function() {
  this.style.background = "var(--blue-mid)";
});
document.getElementById("btnChartReset").addEventListener("mouseout", function() {
  this.style.background = "var(--blue-pale)";
});

function applyChartMonthFilter() {
  var sel = document.getElementById("chartMonthFilter");
  var key = sel.value;
  var btnMTD = document.getElementById("btnChartMTD");
  btnMTD.style.background="var(--card)"; btnMTD.style.color="var(--muted)"; btnMTD.style.borderColor="var(--line)";
  if (key === "all") { renderChart(); }
  else { renderChart((window._chartMonthMap||{})[key]||[]); }
}

function applyChartMTD() {
  var now = new Date();
  var curKey = now.getFullYear()+"-"+String(now.getMonth()+1).padStart(2,"0");
  var monthMap = window._chartMonthMap||{};
  var sel = document.getElementById("chartMonthFilter");
  sel.value = monthMap[curKey] ? curKey : "all";
  var btnMTD = document.getElementById("btnChartMTD");
  btnMTD.style.background="var(--blue)"; btnMTD.style.color="#fff"; btnMTD.style.borderColor="var(--blue)";
  if (monthMap[curKey]) {
    var todayStr=String(now.getDate()).padStart(2,"0")+"/"+String(now.getMonth()+1).padStart(2,"0")+"/"+now.getFullYear();
    renderChart(monthMap[curKey].filter(function(d){ return parseDate(d)<=parseDate(todayStr); }));
  } else { renderChart(); }
}
...
/* ── CHART MONTH FILTER INIT ── */
(function() {
  var allRows = (HIST && HIST.length > 0) ? HIST : DATA;
  var dataKeys = new Set(DATA.map(function(r){ return r.data; }));
  dataKeys.forEach(function(d){ if(!new Set(allRows.map(function(r){return r.data;})).has(d)) allRows=allRows.concat(DATA.filter(function(r){return r.data===d;})); });
  var allDates = [...new Set(allRows.map(function(r){return r.data;}))].sort(function(a,b){return parseDate(a)>parseDate(b)?1:-1;});
  var PT_MONTHS=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];
  var seenMonths=[]; var monthMap={};
  allDates.forEach(function(d){
    var p=d.split("/"); if(p.length<3)return;
    var key=p[2]+"-"+p[1];
    if(!monthMap[key]){monthMap[key]=[];seenMonths.push(key);}
    monthMap[key].push(d);
  });
  var sel=document.getElementById("chartMonthFilter");
  seenMonths.forEach(function(key){
    var parts=key.split("-");
    var label=PT_MONTHS[parseInt(parts[1],10)-1]+" "+parts[0];
    var opt=document.createElement("option"); opt.value=key; opt.textContent=label; sel.appendChild(opt);
  });
  window._chartMonthMap=monthMap;
  var now=new Date(); var curKey=now.getFullYear()+"-"+String(now.getMonth()+1).padStart(2,"0");
  var btnMTD=document.getElementById("btnChartMTD");
  if(monthMap[curKey]&&btnMTD){btnMTD.style.borderColor="var(--blue-mid)";btnMTD.style.color="var(--blue)";btnMTD.style.background="var(--blue-pale)";}
  document.getElementById("chartMonthFilter").addEventListener("change", applyChartMonthFilter);
  document.getElementById("btnChartMTD").addEventListener("click", applyChartMTD);
})();
...
(function(){
  ...
  function clearAllFilters(){
    _newOosFilter=false;_goneOosFilter=false;_semDescontoFilter=false;window._radarBandFilter=null;
    $("fMarca").value="";$("fEstado").value="";$("fSearch").value="";$("fSoOos").checked=false;
    $("cf-marca").value="";$("cf-titulo").value="";$("cf-variante").value="";
    $("cf-estado").value="";$("cf-ref").value="";
    $("cf-desconto").value="";$("cf-oos-dias").value="";
...
def build_oos_desde_map(historico: list, current_rows: list) -> dict:
    """
    Calcula para cada REF a data de início do EPISÓDIO ACTUAL de OOS.

    Regra: só reutiliza o oos_desde histórico se o produto esteve OOS
    ININTERRUPTAMENTE até hoje. Se em algum momento voltou ao stock (is_oos=0),
    o episódio anterior terminou — o oos_desde do novo episódio é hoje.

    Algoritmo:
    1. Para cada ref, ordena o histórico cronologicamente.
    2. Percorre do mais recente para o mais antigo.
    3. Se encontrar is_oos=0 antes de sair do bloco OOS contínuo → episódio novo.
    4. A data do episódio actual = data da entrada OOS mais antiga do bloco contínuo.
    """
    from datetime import datetime as _dt2

    def _parse(s):
        try:    return _dt2.strptime(s, "%d/%m/%Y")
        except: return _dt2.min

    # Agrupa histórico por ref_produto, ordenado cronologicamente
    hist_by_ref: dict = {}
    for row in historico:
        ref = row.get("ref_produto", "")
        if not ref:
            continue
        hist_by_ref.setdefault(ref, []).append(row)
    for ref in hist_by_ref:
        hist_by_ref[ref].sort(key=lambda r: _parse(r.get("data", "")))

    oos_desde = {}

    # Refs que estão OOS na run actual
    current_oos_refs = set(
        row.get("ref_produto", "")
        for row in current_rows
        if row.get("is_oos") and row.get("ref_produto")
    )
    today_str = (current_rows[0].get("data", "") if current_rows else "")

    for ref in current_oos_refs:
        rows = hist_by_ref.get(ref, [])
        if not rows:
            # Produto novo — primeira vez que aparece OOS
            oos_desde[ref] = today_str
            continue

        # Percorre do mais recente para o mais antigo
        # para encontrar o início do bloco OOS contínuo actual
        episode_start = today_str
        for row in reversed(rows):
            if int(row.get("is_oos", 0)):
                # Ainda OOS nesta run — recua o início do episódio
                episode_start = row.get("oos_desde") or row.get("data", "")
            else:
                # Encontrou is_oos=0 — o episódio actual começou depois disto
                break

        oos_desde[ref] = episode_start
...
def generate_dashboard(run_id: str, all_results: list, output_path: str,
    ...
    # Calcula oos_desde
    hist = historico or []
    oos_desde_map = build_oos_desde_map(hist, current_rows)
    ...
    for row in hist:
        ...
        if _seen_run_dates.get(key) == run:
            hist_rows_for_chart.append({
                ...
                "ref_produto": row.get("ref_produto", ""),
            })
    ...
    # NOVOS OOS: diferença real entre hoje e o dia anterior no histórico
    # (consistente com as brand cards e com OOS_hoje - OOS_ontem)
    _today_str = _dt.now().strftime("%d/%m/%Y")

    # Determina o dia anterior a partir do HIST (ordenado cronologicamente)
    def _parse_date_hist(s):
        try:
            return _dt.strptime(s, "%d/%m/%Y")
        except Exception:
            return _dt.min

    hist_dates = sorted(set(r.get("data", "") for r in hist if r.get("data") and r.get("data") != _today_str),
                        key=_parse_date_hist)
    _prev_date = hist_dates[-1] if hist_dates else None

    # OOS de hoje (variantes) — a partir de current_rows
    today_oos_count = sum(1 for r in current_rows if r.get("is_oos"))

    # OOS do dia anterior — a partir do HIST
    if _prev_date:
        prev_oos_count = sum(1 for r in hist_rows_for_chart if r.get("data") == _prev_date and r.get("is_oos"))
    else:
        prev_oos_count = 0

    # new_oos_refs: produtos que ficaram OOS hoje comparando com o dia anterior.
    # Usa o historico.csv directamente (fonte de verdade completa),
    # não o hist_rows_for_chart (que é simplificado para o gráfico e pode ter refs vazios).
    # Pega a última run do dia anterior para ter o estado mais recente desse dia.
    if _prev_date:
        # Encontra o run_id mais recente do dia anterior no historico
        prev_runs = [r.get("run_id", "") for r in hist if r.get("data") == _prev_date and r.get("run_id")]
        _prev_run_id = max(prev_runs) if prev_runs else None
        if _prev_run_id:
            prev_oos_refs = set(
                r.get("ref_produto", "")
                for r in hist
                if r.get("run_id") == _prev_run_id
                and r.get("is_oos")
                and r.get("ref_produto")
            )
        else:
            # Fallback: todas as linhas OOS do dia anterior
            prev_oos_refs = set(
                r.get("ref_produto", "")
                for r in hist
                if r.get("data") == _prev_date and r.get("is_oos") and r.get("ref_produto")
            )
    else:
        prev_oos_refs = set()
    ...
    # "Novo OOS real" = OOS hoje, não estava OOS ontem, E não estava OOS nos últimos 7 dias
    # Isto evita falsos positivos quando o scraper falha 1-2 dias e o produto "reaparece"
    # mas considera genuíno se o produto esteve In Stock por mais de 7 dias seguidos
    from datetime import datetime as _dt_cmp, timedelta as _td_cmp
    def _pd(s):
        try: return _dt_cmp.strptime(s, "%d/%m/%Y")
        except: return _dt_cmp.min
    _today_dt = _pd(_prev_date) + _td_cmp(days=1) if _prev_date else _dt_cmp.now()
    _7days_ago = _today_dt - _td_cmp(days=7)
    # Refs que estiveram OOS em qualquer momento nos últimos 7 dias
    recent_oos_refs = set(
        r.get("ref_produto", "")
        for r in hist
        if r.get("is_oos") and r.get("ref_produto")
        and _pd(r.get("data","")) >= _7days_ago
    )
    new_oos_refs_set  = today_oos_refs - prev_oos_refs - recent_oos_refs  # novo OOS real
    gone_oos_refs_set = prev_oos_refs  - today_oos_refs                    # recuperou hoje

    new_oos_refs  = sorted(new_oos_refs_set)
    gone_oos_refs = sorted(gone_oos_refs_set)
    new_oos_count  = len(new_oos_refs_set)   # card Novos OOS
    gone_oos_count = len(gone_oos_refs_set)  # card Recuperados

    # ── Removidos: estavam ontem (qualquer estado) mas não aparecem hoje ──────
    if _prev_date:
        _prev_run_all = max(
            (r.get("run_id","") for r in hist if r.get("data")==_prev_date and r.get("run_id")),
            default=None
        )
        prev_all_refs = set(
            r.get("ref_produto","")
            for r in hist
            if (r.get("run_id")==_prev_run_all if _prev_run_all else r.get("data")==_prev_date)
            and r.get("ref_produto")
        )
    else:
        prev_all_refs = set()
    today_all_refs   = set(r.get("ref_produto","") for r in current_rows if r.get("ref_produto"))
    # Excluir recuperados (já injectados) e produtos com scraping intermitente (vistos nos últimos 3 dias)
    recent_3d_refs = set(
        r.get("ref_produto","")
        for r in hist
        if r.get("ref_produto") and _pd(r.get("data","")) >= (_today_dt - _td_cmp(days=3))
    )
    removed_refs_set = prev_all_refs - today_all_refs - gone_oos_refs_set - recent_3d_refs
    removed_refs  = sorted(removed_refs_set)
    removed_count = len(removed_refs_set)   # card Removidos

    # ── Injectar recuperados que não aparecem no DATA de hoje ──────────────
    # Agrupa hist por ref (sem filtro de titulo — aceita qualquer row)
    current_refs_in_data = set(r.get("ref_produto","") for r in current_rows)
    missing_recovered = gone_oos_refs_set - current_refs_in_data
    if missing_recovered:
        hist_by_ref_rec = {}
        for row in hist:
            ref = row.get("ref_produto","")
            if ref in missing_recovered:
                hist_by_ref_rec.setdefault(ref, []).append(row)
        today_str_d = current_rows[0].get("data","") if current_rows else run_date
        for ref in sorted(missing_recovered):
            rows_h = sorted(hist_by_ref_rec.get(ref,[]), key=lambda r: r.get("data",""), reverse=True)
            # Pega a melhor row: preferir a que tem titulo, mas aceitar qualquer
            best = next((r for r in rows_h if r.get("titulo")), rows_h[0] if rows_h else {})
            current_rows.append({
                "data":          today_str_d,
                "marca":         best.get("marca",""),
                "titulo":        best.get("titulo") or f"Produto ref {ref}",
                "url":           best.get("url",""),
                "nome_variante": best.get("nome_variante",""),
                "is_oos":        0,
                "ref_produto":   ref,
                "desconto":      best.get("desconto",""),
                "oos_desde":     "",
            })

    # ── Injectar removidos no DATA com flag is_removed ───────────────────────
    if removed_refs_set:
        hist_by_ref_rem = {}
        for row in hist:
            ref = row.get("ref_produto","")
            if ref in removed_refs_set and row.get("titulo"):
                hist_by_ref_rem.setdefault(ref, []).append(row)
        today_str_d2 = current_rows[0].get("data","") if current_rows else run_date
        for ref in removed_refs:
            rows_h = sorted(hist_by_ref_rem.get(ref,[]), key=lambda r: r.get("data",""), reverse=True)
            best = next((r for r in rows_h if r.get("titulo")), rows_h[0] if rows_h else {})
            current_rows.append({
                "data":          today_str_d2,
                "marca":         best.get("marca",""),
                "titulo":        best.get("titulo", f"Produto ref {ref}"),
                "url":           best.get("url",""),
                "nome_variante": best.get("nome_variante",""),
                "is_oos":        -1,
                "ref_produto":   ref,
    ...
    html = html.replace("{{GONE_OOS_COUNT}}",     str(gone_oos_count))
    html = html.replace("{{GONE_OOS_REFS_JSON}}", _json.dumps(gone_oos_refs,       ensure_ascii=False))
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        for cfg in BRANDS:
            try:
                results.append(run_brand(cfg, page))
            except Exception as e:
    ...
    oos_desde_map = build_oos_desde_map(historico, all_current_rows)
