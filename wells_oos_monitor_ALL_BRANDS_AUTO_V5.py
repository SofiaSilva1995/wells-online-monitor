...
/* ═══════════════════════════════════════════════════════
   ...
   ≤768px: tablet/telemóvel | ≤480px: ecrã pequeno
════════════════════════════════════════════════════════*/
@media (max-width: 768px) {
  ...
  /* ── KPIs: 2 colunas + última centrada ── */
  ...
  /* ── Brand cards: 2 colunas em móvel ── */
  ...
  .chk{width:100%}
  .btn-reset,.btn-export{width:100%;justify-content:center}

  /* ── Chart ── */
  .chart-card{padding:14px}
  .chart-wrap{height:200px}

  /* ── Radar de Descontos: empilhar ── */
  ...
  .radar-band-label{font-size:11px}
  .radar-count,.radar-pct{font-size:10px}

  /* ── Tabela ── */
  .tbl-head{flex-direction:column;align-items:flex-start;gap:8px;padding:10px 14px}
  .tbl-head h3{font-size:13px}
  .tbl-head>div{flex-wrap:wrap;gap:6px;width:100%}
  .tbl-head button{font-size:11px;height:28px;padding:3px 12px}
...
/* ── Ecrã muito pequeno (iPhone SE, Android compact) ── */
@media (max-width: 480px) {
  ...
  /* Brand cards: 2 colunas */
  .brand-grid{grid-template-columns:1fr 1fr;gap:6px}
  .bc-oos{font-size:20px}

  /* Tabela: esconder colunas menos críticas */
  /* Manter: Estado, Marca, Produto, URL */
  /* Esconder: Data, Variante, REF, Desconto */
  thead th:nth-child(1),tbody td:nth-child(1){display:none} /* Data */
  thead th:nth-child(4),tbody td:nth-child(4){display:none} /* Variante */
  thead th:nth-child(6),tbody td:nth-child(6){display:none} /* REF */
  thead th:nth-child(7),tbody td:nth-child(7){display:none} /* Desconto */
...
document.getElementById("btnChartReset").addEventListener("click", function() {
  // Resetar filtro de mês
  var sel = document.getElementById("chartMonthFilter");
  if (sel) sel.value = "all";
  // Resetar botão MTD
  var btnMTD = document.getElementById("btnChartMTD");
  if (btnMTD) {
    btnMTD.style.background = "var(--card)";
    btnMTD.style.color = "var(--muted)";
    btnMTD.style.borderColor = "var(--line)";
  }
  // Re-renderizar com todos os dados e todos os datasets visíveis
  renderChart();
  if (_chart) {
    var count = _chart.data.datasets.length;
    for (var i = 0; i < count; i++) { _chart.show(i); }
    _chart.update();
  }
});
...
function applyChartMonthFilter() {
  ...
  btnMTD.style.color = "var(--muted)";
  btnMTD.style.borderColor = "var(--line)";
  btnMTD.style.fontWeight = "700";

  if (key === "all") {
    renderChart();
  } else {
    var dates = (window._chartMonthMap || {})[key] || [];
    renderChart(dates);
  }
}
...
function applyChartMTD() {
  ...
  // Realçar botão MTD
  var btnMTD = document.getElementById("btnChartMTD");
  btnMTD.style.background = "var(--blue)";
  btnMTD.style.color = "#fff";
  btnMTD.style.borderColor = "var(--blue)";

  if (monthMap[curKey]) {
    // Filtrar só os dias do mês atual até hoje
    var todayStr = String(now.getDate()).padStart(2,"0") + "/" +
                   String(now.getMonth()+1).padStart(2,"0") + "/" +
                   now.getFullYear();
    var mtdDates = monthMap[curKey].filter(function(d) {
      return parseDate(d) <= parseDate(todayStr);
    });
    renderChart(mtdDates);
  } else {
    renderChart();
  }
}
...
/* ── CHART MONTH FILTER — init depois do boot para parseDate estar disponível ── */
(function() {
  var allRows = (HIST && HIST.length > 0) ? HIST : DATA;
  var dataKeys = new Set(DATA.map(function(r){ return r.data; }));
  var histKeys = new Set(allRows.map(function(r){ return r.data; }));
  ...
  // Realçar MTD se mês atual tem dados
  var now = new Date();
  var curKey = now.getFullYear() + "-" + String(now.getMonth()+1).padStart(2,"0");
  var btnMTD = document.getElementById("btnChartMTD");
  if (monthMap[curKey]) {
    btnMTD.style.borderColor = "var(--blue-mid)";
    btnMTD.style.color = "var(--blue)";
    btnMTD.style.background = "var(--blue-pale)";
  }

  // Ligar eventos aqui — funções já estão definidas neste ponto
...
def generate_dashboard(run_id: str, all_results: list, output_path: str,
    ...
    new_oos_refs_set  = today_oos_refs - prev_oos_refs   # entrou em OOS hoje
