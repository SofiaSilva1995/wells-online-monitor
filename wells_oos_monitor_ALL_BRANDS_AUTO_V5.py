...
def generate_dashboard(run_id: str, all_results: list, output_path: str,
    ...
    # Usa comparação real entre runs (não oos_desde) — garante consistência entre
    # o contador do card e as linhas filtradas na tabela.
    if _prev_date:
        prev_oos_refs = set(
            r.get("ref_produto", "")
            for r in hist_rows_for_chart
            if r.get("data") == _prev_date and r.get("is_oos")
        )
    else:
        prev_oos_refs = set()
    ...
    new_oos_refs_set  = today_oos_refs - prev_oos_refs   # entrou em OOS
    gone_oos_refs_set = prev_oos_refs  - today_oos_refs  # recuperou
