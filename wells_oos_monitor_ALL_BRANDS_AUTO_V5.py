...
def run_brand(cfg: Dict, page) -> Dict:
    ...
    if multi_brand:
        ...
        # FILTRO INTELIGENTE para Oral Care
        # 1. Produtos individuais (Elugel, Parodium) sempre passam
        # 2. Outros: só aceita se URL contém marca permitida
        # 3. Lista de exclusão para marcas conhecidas não-PF
        ...
        allowed_brands = ['eludril', 'elgydium', 'arthrodont', 'elugel', 'parodium']
        
        # Marcas conhecidas NÃO Pierre Fabre para rejeitar
        excluded_brands = [
            'lancome', 'nyx', 'it-cosmetics', 'cosmetics-do-it',
        ...
        for url in all_urls_collected:
            ...
            # 1. Produtos individuais sempre passam
            if any(prod_id in url for prod_id in individual_products):
                filtered_urls.add(url)
                continue
            
            # 2. Rejeita marcas conhecidas não-PF
            if any(excluded in url_lower for excluded in excluded_brands):
                log(f"{label}: Produto rejeitado (marca excluída): {url}")
                continue
            
            # 3. Aceita se contém marca permitida
            if any(brand in url_lower for brand in allowed_brands):
                filtered_urls.add(url)
            else:
                log(f"{label}: Produto rejeitado (marca não reconhecida): {url}")
        
        rejected_count = len(all_urls_collected) - len(filtered_urls)
        if rejected_count > 0:
            log(f"{label}: {rejected_count} produtos rejeitados por filtros")
